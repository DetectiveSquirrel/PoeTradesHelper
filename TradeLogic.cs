...
public class TradeLogic
{
    ...
    public ConcurrentDictionary<int, TradeEntry> TradeEntries { get; } = new();
    ...
    {
        if (_settings.RemoveDuplicatedTrades.Value && TradeEntries.Any(x => x.Value.PlayerNick == message.Nick && x.Value.Message == message.Message))
        {
            return;
        }
        ...
        var tradeEntry = new TradeEntry(itemAmount, itemName, message.Nick, currencyType, currencyAmount, message.MessageType == MessageType.From, EntryUniqueIdCounter, message.Message);
...
{
    public TradeEntry(string itemAmount, string itemName, string playerNick, string currencyType, string currencyAmount, bool incomingTrade, int uniqueId, string message)
    {
